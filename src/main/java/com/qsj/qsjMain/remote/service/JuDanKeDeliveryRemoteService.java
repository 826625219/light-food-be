...
@Slf4j
@Service
public class JuDanKeDeliveryRemoteService {
    ...
    public IJuDanKeDeliveryRemoteService getService() {

        return new Retrofit.Builder().baseUrl(profileVariableConfig.getJuDanKeDeliveryPrefix()).client(client)
                .addConverterFactory(
                        GsonConverterFactory.create(new GsonBuilder().create()))
                .build()
                .create(IJuDanKeDeliveryRemoteService.class);
    }
